...
train_data = pd.read_csv(tens_path + "AF_manner_sample_train.csv", sep=",", header=None)
valid_data = pd.read_csv(tens_path + "AF_manner_sample_test.csv", sep=",", header=None)
# train_n = dataframe.shape[0]
train_n = 20000

# train_n = dataframe.shape[0]
# assuming label is in final column
feat_n = train_data.shape[1] - 1
#val_prop = 0.1111111
...
def train_nn_classification_model(learning_rate, steps, batch_size, hidden_units, training_examples, training_targets, validation_examples, validation_targets):
    ...
    classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=n_classes[af], hidden_units=hidden_units, optimizer=my_optimizer, config=tf.contrib.learn.RunConfig(keep_checkpoint_max=1), model_dir=tens_path + af + "_model2")
    ...
    with open(tens_path + af + "_validation_cm2.csv", "w") as f:
