...
print("Loading CELEX")
# celex = {}
# with codecs.open(tens_path + "other/DPW.CD", "r", "utf-8") as f:
#    for line in f:
#        l_list = line[:-1].split("\\")
#        word = l_list[1]
#        lem_i = l_list[3]
...
#            celex_morph[celex_id] = {"tag": morph_tag2, "lem": cel_lem}
...
timbl = {}
with codecs.open(tens_path + "other/pl_type_timbl.csv", "r", "utf-8") as f:
    for line in f:
        word = line[:-1].split(",")[0]
...
def getPOS(rt, s_ind, w_ind, wrd, chunk_i, word_class, s_type, w_ph):
    ...
    if len(pw_list) == 0:
        ...
        print(".".join([rt.attrib["ref"], str(s_ind), str(w_ind)]))
...
def parseLine(f_path, from_time, to_time, tier, new_file, word, w_cl, type_of_s, word_phons, word_stress):
    ...
    if new_file:
        if component in ["c", "d"] and language == "vl":
...
#    en_freq = subtlex2[en_word]["N"] if (en_word in subtlex2) else 0
    ...
    # get timbl probability
    if lem in timbl:
        if "S" in timbl[lem]:
            timbl_prob = timbl[lem]["S"]
        else:
            timbl_prob = "NA"
    else:
        timbl_prob = "NA"
    return syntax_feats, voice_feat, lem, rel_freq, str(s_freq), str(en_freq), pl_prop, str(lem_freq), rel_freq_pl, timbl_prob
...
#                print("core " + core_num + ": " + line[:-1].encode("utf-8"))
                ...
                type_s = line_l[18]
                if type_s != "PL":
                    g.write(line[:-1] + ",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA\n")
                    continue
#                print(line_l)
                print("core " + core_num + ": " + line[:-1].encode("utf-8"))
                num_syl = line_l[33]
                ...
                syn_feats, voice, lemma, relfreq, pl_s_freq, pl_en_freq, pl_propo, lemma_freq, relfreq_pl, timbl_s_prob = parseLine(file_path, c_start, c_end, tg_tier, new, w, word_cl, type_s, w_phons, stressed)
                g.write(",".join(line_l[:34] + [word_stress] + line_l[35:] + syn_feats + [voice, lemma] + relfreq + [pl_s_freq, pl_en_freq, pl_propo, lemma_freq, relfreq_pl, timbl_s_prob]) + "\n")
...
def multiProcess():
    ...
    with codecs.open(tens_path + "cgn/synvoirelPL_s_comb_comp-" + component + "_timbl.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
