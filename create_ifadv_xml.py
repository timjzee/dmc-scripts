...
def make_tag_files(rt):
    ...
    with codecs.open(tens_path + "POS_IFADV/" + file_n + ".pos", "r", "utf-8") as f:
        spreker1 = []
        spreker2 = []
        spreker = 1
        for l in f:
...
#            print(t_info, etree.tostring(tag_l), t_num)
            ...
            pau.append(pw)
        tag.append(pau)
    with codecs.open(tens_path + "Annotations/tag/" + wavpaths[counter] + ".tag", "w", "utf-8") as f:
...
#            print("S2", s1_lab, s1_i, "/", len(s1_tier_l), s2_lab, s2_i, "/", len(s2_tier_l))
        ...
        for s in sentences:
            root.append(s)
    with codecs.open(tens_path + "Annotations/skp-ort/" + wavpaths[counter] + ".skp", "w", "utf-8") as f:
