...
gs_df$v_classifier_average_score <- (gs_df$v_perc20_TR_NN + gs_df$v_variant_accuracy + gs_df$v_variant_macro_Fscore) / 3
gs_df$v_classifier_average_score_TZ <- (gs_df$v_perc20_TR_NN + gs_df$v_variant_accuracy_TZ + gs_df$v_variant_macro_Fscore_TZ) / 3

gs_df$t_classifier_average_score <- (gs_df$t_perc20_TR_NN + gs_df$t_variant_accuracy + gs_df$t_variant_macro_Fscore) / 3
...
plot(best_ind - 0.25*((nrow(gs_df)/2) %/% 100), gs_df$v_perc20_TR_NN[best_ind], xlab = "", ylab = "", xaxt='n', type = "h", ylim = c(min(gs_df$t_perc20_TR_NN[best_ind])-0.002, max(gs_df$v_perc20_TR_TS[1],gs_df$v_perc20_TR_NN[1])+0.002))
points(best_ind + 0.25*((nrow(gs_df)/2) %/% 100), gs_df$t_perc20_TR_NN[best_ind], type = "h", col="red")
text(best_ind, par("usr")[3], labels = gs_df$gs_name[best_ind], srt = 45, adj = c(1.1,1.1), xpd = TRUE, cex=0.4)

abline(h = gs_df$v_perc20_TR_TS[1], lty="dashed", col="black")
abline(h = gs_df$t_perc20_TR_TS[1], lty="dashed", col="red")

# best 100
best_ind <- 1:100
plot(best_ind-0.25, gs_df$v_perc20_TR_NN[best_ind], xlab = "", ylab = "", xaxt='n', type = "h", ylim = c(min(gs_df$t_perc20_TR_NN[best_ind])-0.002, max(gs_df$v_perc20_TR_TS[1],gs_df$v_perc20_TR_NN[1])+0.002))
points(best_ind + 0.25, gs_df$t_perc20_TR_NN[best_ind], type = "h", col="red")
text(best_ind, par("usr")[3], labels = gs_df$gs_name[best_ind], srt = 45, adj = c(1.1,1.1), xpd = TRUE, cex=0.4)
...
v_cm <- table(best[vali_indices,]$TS_variant, best[vali_indices,]$TR_variant, dnn = c("TS", "TR"))
...
v_human_average_score <- (gs_df$v_perc20_TR_TS[1] + v_Accuracy + v_macro.Fscore) / 3

t_cm <- table(best[test_indices,]$TS_variant, best[test_indices,]$TR_variant, dnn = c("TS", "TR"))
...
t_human_average_score <- (gs_df$t_perc20_TR_TS[1] + t_Accuracy + t_macro.Fscore) / 3
...
v_human_average_score_TZ <- (gs_df$v_perc20_TR_TS[1] + v_Accuracy_TZ + v_macro.Fscore_TZ) / 3
...
t_human_average_score_TZ <- (gs_df$t_perc20_TR_TS[1] + t_Accuracy_TZ + t_macro.Fscore_TZ) / 3

v_cm_NN <- table(best[vali_indices,]$classifier_variant, best[vali_indices,]$suffix_variant, dnn = c("NN", "TZ"))
...
t_cm_NN <- table(best[test_indices,]$classifier_variant, best[test_indices,]$suffix_variant, dnn = c("NN", "TZ"))
...
# vali set
best_TR_NN_perc <- c(gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_schwa_perc20_TR_NN[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_nasal_perc20_TR_NN[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_nasalization_perc20_TR_NN[1])
TR_TS_perc <- c(gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_schwa_perc20_TR_TS[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_nasal_perc20_TR_TS[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$v_nasalization_perc20_TR_TS[1])
...
# test set
best_TR_NN_perc <- c(gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_schwa_perc20_TR_NN[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_nasal_perc20_TR_NN[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_nasalization_perc20_TR_NN[1])
TR_TS_perc <- c(gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_schwa_perc20_TR_TS[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_nasal_perc20_TR_TS[1], gs_df[order(gs_df$v_classifier_average_score_TZ, decreasing = T),]$t_nasalization_perc20_TR_TS[1])
prec_labels <- c("schwa", "nasal", "nasalization")
...
# vali set
best_TZ_NN_perc <- c(v_macro.Fscore_NN, v_Accuracy_NN)
TR_TZ_perc <- c(v_macro.Fscore_TZ, v_Accuracy_TZ)
prec_labels <- c("Accuracy", "Macro F-Score")


plot(NULL, xlim = c(0.5,3.5), ylim = c(0,1), xaxt="n", ylab="Percentage", xlab = "Classifier", main = "Validation set")
for (i in 1:2){
  lines(x = c(i - 0.1, i - 0.1), y = c(0, TR_TZ_perc[i]), col="black", lwd=4)
  lines(x = c(i + 0.1, i + 0.1), y = c(0, best_TZ_NN_perc[i]), col="cadetblue", lwd=4)
}
lines(x = c(2.8, 3), y = c(1,1), col="black", lwd=4)
text(x = 3.3, y = 1, "Human")
lines(x = c(2.8, 3), y = c(.9,.9), col="cadetblue", lwd=4)
...
# test set
best_TZ_NN_perc <- c(t_macro.Fscore_NN, t_Accuracy_NN)
TR_TZ_perc <- c(t_macro.Fscore_TZ, t_Accuracy_TZ)
prec_labels <- c("Accuracy", "Macro F-Score")
