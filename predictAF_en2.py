...
classifiers = {
    "n": tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=2, hidden_units=[300], model_dir=tens_path + "n_8k_model"),
    "schwa": tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=2, hidden_units=[300], model_dir=tens_path + "schwa_8k_model")
}

frag_fol = "d"
file_paths = glob.glob(tens_path + "pred_fragments_n/" + frag_fol + "/*.wav")
# file_paths = [
...
tg_paths = glob.glob(tens_path + "pred_textgrids_n/" + tg_fol + "/*.IntensityTier")

print("number of wav files before: ", len(file_paths))
for tg_path in tg_paths:
    fp1 = "/".join(tg_path.split("/")[:-1]) + "/"
    fp1 = re.sub(r"pred_textgrids", "pred_fragments", fp1)
...
#                print(unseen_samples.shape, np.array(new_row).shape)
        ...
        summary_text.to_csv(tens_path + "pred_mfcc_n/" + corpus + "/" + fragment_id + "_sum.csv", float_format="%.2f")
...
#                print(af_probs)
                with open(tens_path + "pred_textgrids_n/" + corpus + "/" + fragment_id + "_" + af + ".IntensityTier", "w") as f:
...
#                    max_time = float(end_time)
            ...
            tg.append(tier)
        with open(tens_path + "pred_textgrids_n/" + corpus + "/" + fragment_id + ".TextGrid", "w") as f:
            tg.write(f)
