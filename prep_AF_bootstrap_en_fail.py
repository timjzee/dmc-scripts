...
def getFeatureLabel(frame_start, frame_end, feature, seg_index, segments, words, word_syllables, word_segments, comment_intervals):
    ...
    if feature == "@":
        ...
        if phon.strip("#") == "@" and num_rem_syls <= 1 and final_n:
            return -1
        if phon.strip("#") == "@" and num_rem_syls <= 1 and re.search(r'ns$', word_str):  # 26aug.txt: exclude telkens, tijdens
            return -1
...
#            return -1
        ...
        # negative
            ...
            if prev_phon_str not in ["n", "m", "N"] and (prev_phon[-1] != "#" or phon == "w" or (phon in ["b", "d", "g"] and prev_phon_str in vowels)) and not (prev_phon_str in ["t", "d", "j"] and prev_prev_phon_str == "n") and not (prev_phon_str in ["p", "b"] and prev_prev_phon_str == "m") and not (prev_phon_str in ["k", "g"] and prev_prev_phon_str == "N") and next_phon_str not in ["n", "m", "N"] and (phon[-1] != "#" or next_phon_str in ["SIL", "", "b", "d", "g"]) and not (phon.strip("#") in vowels and next_phon_str == "@" and num_rem_syls <= 2 and final_n) and not (phon.strip("#") not in vowels and next_phon_str == "@" and num_rem_syls <= 1 and final_n):
                ...
                used_s = round(segments.intervals[seg_index].minTime, 3)
                used_e = used_s + prop_dur
            # 26aug.txt: changed so word-initial /w/ and voiced plosives following a vowel are fully used
            elif prev_phon_str not in ["n", "m", "N"] and (prev_phon[-1] != "#" or phon == "w" or (phon in ["b", "d", "g"] and prev_phon_str in vowels)) and not (prev_phon_str in ["t", "d", "j"] and prev_prev_phon_str == "n") and not (prev_phon_str in ["p", "b"] and prev_prev_phon_str == "m") and not (prev_phon_str in ["k", "g"] and prev_prev_phon_str == "N"):
                prop_used = 0.7
                prop_dur = int_dur * prop_used
                used_s = round(segments.intervals[seg_index].minTime, 3)
                used_e = used_s + prop_dur
            # 26aug.txt: changed so pre-silence and pre-voiced plosive segments are fully used
            elif next_phon_str not in ["n", "m", "N"] and (phon[-1] != "#" or next_phon_str in ["SIL", "", "b", "d", "g"]) and not (phon.strip("#") in vowels and next_phon_str == "@" and num_rem_syls <= 2 and final_n) and not (phon.strip("#") not in vowels and next_phon_str == "@" and num_rem_syls <= 1 and final_n):  # exclude e.g. [j] in koeien
                prop_used = 0.7
                prop_dur = int_dur * prop_used
                used_e = round(segments.intervals[seg_index].maxTime, 3)
                used_s = used_e - prop_dur
        ...
        if phon in vowels and next_phon_str in ["n", "m", "N"]:  # be aware, this now includes schwa in bananen
            if phon == "@" and num_rem_syls <= 1 and re.search(r'ns$', word_str):
                return -1
            elif prev_phon in ["n", "m", "N"] and not (final_n and next_phon[-1] == "#"):
                prop_used = 1
                prop_dur = int_dur * prop_used
                used_s = round(segments.intervals[seg_index].minTime, 3)
                used_e = used_s + prop_dur
