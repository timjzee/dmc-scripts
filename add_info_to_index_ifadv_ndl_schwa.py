...
def getNDLphons(orig_path, c_start, c_end, spkr, word_index, wndw_start, wndw_end):
    """Calls a Praat script which finds the chunk and returns info on phonetic context."""
    output = subprocess.check_output([tz_path + "praat_nogui", "--run", tz_path + "GitHub/dmc-scripts/getNDLphons.praat", orig_path, c_start, c_end, spkr, str(word_index), str(wndw_start), str(wndw_end), "IFADVcorpus/kaldi_annot/v2/"]).decode("utf-8")[:-1].split(" ")
#    print(output)
    return output
...
#    else:
    ...
    with codecs.open(tens_path + "IFADVcorpus/all_en" + core_num + ".csv", "w", "utf-8") as g:
        with codecs.open(tens_path + "IFADVcorpus/ndl_en" + core_num + ".csv", "w", "utf-8") as h:
...
def multiProcess():
    ...
    with codecs.open(tens_path + "IFADVcorpus/all_en_comb_ifadv_ndl.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
            with codecs.open(tens_path + "IFADVcorpus/all_en" + core_n + ".csv", "r", encoding="utf-8") as f:
                for fln, f_line in enumerate(f, 1):
                    if not (core > 0 and fln == 1):
                        g.write(f_line)
    with codecs.open(tens_path + "IFADVcorpus/ndl_en_ifadv.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
            with codecs.open(tens_path + "IFADVcorpus/ndl_en" + core_n + ".csv", "r", encoding="utf-8") as f:
                for fln, f_line in enumerate(f, 1):
                    if not (core > 0 and fln == 1):
                        g.write(f_line)
