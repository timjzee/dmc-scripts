...
s_df = read.csv(paste(f_path, "all_s_comb_d_vl_ndl.csv", sep = ""))
# if non cgn corpus
s_df$language = "vl"
...
s_sub_df = s_df[s_df$language == "vl" 
#                & s_df$overlap == FALSE 
...
write.table(s_sub_df[ ,c("wav", "chan", "chunk_start", "chunk_end", "tier", "word_chunk_i", 
                         ...
                         "overlap")], 
            file = paste(f_path, "comp-d_s_vl_ndl.csv", sep = ""), row.names=FALSE, col.names=TRUE, sep=",", quote = FALSE)
