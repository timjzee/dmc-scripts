...
# filepaths = ["/vol/tensusers/timzee/cgn/Annotations/ort/comp-a/fn007979.ort"]
# wavpaths = ["fn007979"]
...
def make_tag_files(rt, spkrs):
    ...
    with codecs.open(tens_path + "Annotations/pos/comp-a/" + file_n + ".pos", "r", "utf-8") as f:
        for l in f:
            if file_n in l:
                for spkr in spkrs:
                    if spkr in l:
...
#                print(etree.tostring(tag_l))
    ...
    #            assert t_info[0] in tag_l.get("w")
                ...
                pau.append(pw)
            tag.append(pau)
    with codecs.open(tens_path + "Annotations/tag/comp-a/" + wavpaths[counter] + ".tag", "w", "utf-8") as f:
...
def make_xml_files(fp):
    ...
    with gzip.open(cgn_path + "xml/skp-ort/comp-a/nl/" + fp + ".skp.gz") as h:
        skp_gz = h.read()
