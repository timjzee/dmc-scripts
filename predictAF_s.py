...
classifiers = {
    "s": tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=2, hidden_units=[300, 150], model_dir=tens_path + "s_model_comp-o")
}
...
#            print(unseen_samples.shape, np.array(new_row).shape)
    ...
    summary_text.to_csv(tens_path + "pred_mfcc/sep_mod/" + fragment_id + "_sum.csv", float_format="%.2f")
...
#            print(af_probs)
            with open(tens_path + "pred_textgrids/sep_mod/" + fragment_id + "_" + af + ".IntensityTier", "w") as f:
...
#                max_time = float(end_time)
        ...
        tg.append(tier)
    with open(tens_path + "pred_textgrids/sep_mod/" + fragment_id + "_" + af + ".TextGrid", "w") as f:
        tg.write(f)
