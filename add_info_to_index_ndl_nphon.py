...
component = "o"
input_file_path = tens_path + "cgn/cgn_index_o_final_nl.txt"
...
def getNDLphons(orig_path, c_start, c_end, spkr, word_index, wndw_start, wndw_end):
    """Calls a Praat script which finds the chunk and returns info on phonetic context."""
    output = subprocess.check_output([tz_path + "praat_nogui", "--run", tz_path + "GitHub/dmc-scripts/getNDLphons2.praat", orig_path, c_start, c_end, spkr, str(word_index), str(wndw_start), str(wndw_end), "cgn/kaldi_annot/v2/comp-", str(n_phones)]).decode("utf-8")[:-1].split(" ")
#    print(output)
    return output
...
def multiProcess():
    ...
    with codecs.open(tens_path + "cgn/all_s_comb_" + component + "_ndl_tri.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
            with codecs.open(tens_path + "cgn/all_s" + core_n + ".csv", "r", encoding="utf-8") as f:
                for fln, f_line in enumerate(f, 1):
                    if not (core > 0 and fln == 1):
                        g.write(f_line)
    with codecs.open(tens_path + "cgn/ndl_comp-" + component + "_tri.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
