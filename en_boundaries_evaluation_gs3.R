...
gs_df$t_variant_accuracy_MW <- rep(0.1, num_gs)
gs_df$t_variant_macro_Fscore_MW <- rep(0.1, num_gs)
...
#  print(gs_name)
  ...
  ### schwa
  ## Human-Human
  bounds$TR_MW_e_start_b <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_e_start_b, bounds$MW_e_start_b)
  schwa_perc20_TR_MW_startb <- NROW(na.omit(bounds[abs(bounds$TR_MW_e_start_b) < 0.02,c("TR_MW_e_start_b")])) / NROW(bounds[,c("TR_MW_e_start_b")])
  bounds$TR_MW_e_start_e <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_e_start_e, bounds$MW_e_start_e)
  schwa_perc20_TR_MW_starte <- NROW(na.omit(bounds[abs(bounds$TR_MW_e_start_e) < 0.02,c("TR_MW_e_start_e")])) / NROW(bounds[,c("TR_MW_e_start_e")])
  bounds$TR_MW_e_end_b <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_e_end_b, bounds$MW_e_end_b)
  schwa_perc20_TR_MW_endb <- NROW(na.omit(bounds[abs(bounds$TR_MW_e_end_b) < 0.02,c("TR_MW_e_end_b")])) / NROW(bounds[,c("TR_MW_e_end_b")])
  bounds$TR_MW_e_end_e <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_e_end_e, bounds$MW_e_end_e)
  schwa_perc20_TR_MW_ende <- NROW(na.omit(bounds[abs(bounds$TR_MW_e_end_e) < 0.02,c("TR_MW_e_end_e")])) / NROW(bounds[,c("TR_MW_e_end_e")])
  bounds_TR_MW_e_long <- c(bounds$TR_MW_e_start_b, bounds$TR_MW_e_start_e, bounds$TR_MW_e_end_b, bounds$TR_MW_e_end_e)
  schwa_perc20_TR_MW <- NROW(na.omit(bounds_TR_MW_e_long[abs(bounds_TR_MW_e_long) < 0.02])) / NROW(bounds_TR_MW_e_long)
  ## Human-Computer
  ...
  ### nasal
  ## Human-Human
  bounds$TR_MW_n_start_b <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_n_start_b, bounds$MW_n_start_b)
  nasal_perc20_TR_MW_startb <- NROW(na.omit(bounds[abs(bounds$TR_MW_n_start_b) < 0.02,c("TR_MW_n_start_b")])) / NROW(bounds[,c("TR_MW_n_start_b")])
  bounds$TR_MW_n_start_e <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_n_start_e, bounds$MW_n_start_e)
  nasal_perc20_TR_MW_starte <- NROW(na.omit(bounds[abs(bounds$TR_MW_n_start_e) < 0.02,c("TR_MW_n_start_e")])) / NROW(bounds[,c("TR_MW_n_start_e")])
  bounds$TR_MW_n_end_b <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_n_end_b, bounds$MW_n_end_b)
  nasal_perc20_TR_MW_endb <- NROW(na.omit(bounds[abs(bounds$TR_MW_n_end_b) < 0.02,c("TR_MW_n_end_b")])) / NROW(bounds[,c("TR_MW_n_end_b")])
  bounds$TR_MW_n_end_e <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_n_end_e, bounds$MW_n_end_e)
  nasal_perc20_TR_MW_ende <- NROW(na.omit(bounds[abs(bounds$TR_MW_n_end_e) < 0.02,c("TR_MW_n_end_e")])) / NROW(bounds[,c("TR_MW_n_end_e")])
  bounds_TR_MW_n_long <- c(bounds$TR_MW_n_start_b, bounds$TR_MW_n_start_e, bounds$TR_MW_n_end_b, bounds$TR_MW_n_end_e)
  nasal_perc20_TR_MW <- NROW(na.omit(bounds_TR_MW_n_long[abs(bounds_TR_MW_n_long) < 0.02])) / NROW(bounds_TR_MW_n_long)
  ## Human-Computer
  ...
  ### nasalization
  ## Human-Human
  bounds$TR_MW_N_b <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_N_b, bounds$MW_N_b)
  nasalization_perc20_TR_MW_b <- NROW(na.omit(bounds[abs(bounds$TR_MW_N_b) < 0.02,c("TR_MW_N_b")])) / NROW(bounds[,c("TR_MW_N_b")])
  bounds$TR_MW_N_e <- mapply(function(x, y) ifelse(is.na(x) && is.na(y), 0, x - y), bounds$TR_N_e, bounds$MW_N_e)
  nasalization_perc20_TR_MW_e <- NROW(na.omit(bounds[abs(bounds$TR_MW_N_e) < 0.02,c("TR_MW_N_e")])) / NROW(bounds[,c("TR_MW_N_e")])
  bounds_TR_MW_N_long <- c(bounds$TR_MW_N_b, bounds$TR_MW_N_e)
  nasalization_perc20_TR_MW <- NROW(na.omit(bounds_TR_MW_N_long[abs(bounds_TR_MW_N_long) < 0.02])) / NROW(bounds_TR_MW_N_long)
  ## Human-Computer
  ...
  bounds_TR_MW_long <- c(bounds_TR_MW_e_long, bounds_TR_MW_n_long, bounds_TR_MW_N_long)
  perc20_TR_MW <- NROW(na.omit(bounds_TR_MW_long[abs(bounds_TR_MW_long) < 0.02])) / NROW(bounds_TR_MW_long)
...
write.csv(gs_df, file = paste(f_path, "gs_summary_mw.csv", sep = ""),quote = F, row.names = F)
