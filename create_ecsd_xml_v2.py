...
def getSentence(tier, start_i, sen_i):
    ...
    while (int_lab[-1] not in [".", "?", "!"] and int_i + 1 < len(tier_l)):
        ...
        if len(int_lab) == 0:
            int_lab = " "
        if int_lab != " " and int_lab[-1] not in [".", "?", "!", ",", ":", ";"]:
            int_lab += "."
...
#        int_ort = re.sub(r'[\.!]*[!]+[\.!]*', '!', int_ort)   # replace combos including at least 1 '!'
#        int_ort = re.sub(r'[\.!?]*[?]+[\.!?]*', '?', int_ort)  # replace combos including at least 1 '?'
#        int_ort = re.sub(r'\.+', '.', int_ort)   # replace clusters of '.' with a single '.'
#        int_ort = re.sub(r'(?<=[A-Za-z])&(?=[A-Za-z])', ' & ', int_ort)
        int_ort = re.sub(r'\\[vVoO]', '', int.mark)  # *u
        int_ort = re.sub(r'[!?\.,:;](?=[A-Za-z])', lambda m: m.group() + " ", int_ort)
        int_ort = re.sub(r'(?<=[A-Za-z])&(?=[A-Za-z])', ' & ', int_ort)
        int_ort = re.sub(r'[\s]+', ' ', int_ort)
        int_ort = re.sub(r'\\\*', '', int_ort.strip(" "))         # *a
        int_ort = re.sub(r'\\-', ' ', int_ort)                    # spelling
        int_ort = re.sub(r'"', '', int_ort)
        int_ort = re.sub(r'[ ]+(?=[.,:;?!])', "", int_ort)
        int_ort = re.sub(r'[\.!]*[!]+[\.!]*', '!', int_ort)   # replace combos including at least 1 '!'
        int_ort = re.sub(r'[\.!?]*[?]+[\.!?]*', '?', int_ort)  # replace combos including at least 1 '?'
        int_ort = re.sub(r'\.+', '.', int_ort)   # replace clusters of '.' with a single '.'
        int_ort = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\xff]', '', int_ort)
        words = []
...
#            w = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\xff]', '', w)
            if w not in ["", " "]:
                if re.search(r'[!?.]', w):
                    for c, sw in enumerate(re.split(r'[!?.]', w), 1):
                        if sw not in ["", '"']:
                            if c != len(re.split(r'[!?.]', w)):
                                words.append(sw + ".")
                            else:
                                words.append(sw)
                else:
...
def make_tag_files(rt):
    ...
    with codecs.open(tens_path + "Annotations/pos2/" + pair_n.upper() + "/" + file_n_lc + ".pos", "r", "utf-8") as f:
        spreker1 = []
        spreker2 = []
        spreker = 0
        for l in f:
...
#        print(n)
#        print(i)
#        print(etree.tostring(ort_spreker1[n]))
#        print("\n")
...
#    print(etree.tostring(ort_spreker1[-1]), "\n", spreker1[-1])
    ...
    for sn in rt:
        ...
        s_tags = [i for i in s_tags if (i.split("\t")[1] != "LET()" or i.split("\t")[0] == "&")]  # we want to skip all interpunction except for &
...
#            print(t_info, etree.tostring(tag_l), t_num)
            ...
            pau.append(pw)
        tag.append(pau)
    with codecs.open(tens_path + "Annotations/tag2/" + pair_n.upper() + "/" + file_n_lc + ".tag", "w", "utf-8") as f:
...
#            print("S2", s1_lab, s1_i, "/", len(s1_tier_l), s2_lab, s2_i, "/", len(s2_tier_l))
        ...
        for s in sentences:
            root.append(s)
    with codecs.open(tens_path + "Annotations/skp-ort2/" + pair_n.upper() + "/" + file_n_lc + ".skp", "w", "utf-8") as f:
