...
if os$ == "macintosh"
    tens_path$ = "/Volumes/tensusers/timzee/classifier_evaluation/en/"
    wrd_tg_path$ = "/Volumes" + wrd_path$
    kal_tg_path$ = "/Volumes" + kal_path$
else
    tens_path$ = "/vol/tensusers/timzee/classifier_evaluation/en/"
    wrd_tg_path$ = "/vol" + wrd_path$
    kal_tg_path$ = "/vol" + kal_path$
endif
...
index_path$ = tens_path$ + input_name$ + ".csv"
...
Rename: "chunks"
Append column: "cgn_tran"
Append column: "cgn_type"
Append column: "kal_tran"
Append column: "kal_type"
Append column: "cgn_start"
Append column: "cgn_end"
Append column: "preselect_type"
...
for id from 1 to num_chunks
    ...
    if f_name$ != old_name$
        ...
        ## Load TextGrids
        Read from file: kal_tg_path$ + f_path$ + ".awd"
        Rename: f_name$ + "_kal"
...
#        word$ = word_array$[word_i]
        ...
        if word_i == word_chunk_i
            ...
            if wrd_tran$ == "[SPN]" or wrd_tran$ == "[]"
                ...
                cgn_tran$ = ""
                cgn_type$ = ""
                if (getSegments.phons$[n_phon] == "n" or getSegments.phons$[n_phon] == "m" or getSegments.phons$[n_phon] == "N") and n_phon > 1
                    if getSegments.phons$[n_phon-1] == "@"
                        cgn_type$ = "@n"
                    elsif index(medeklinkers$, getSegments.phons$[n_phon-1]) != 0
                        cgn_type$ = "n"
                    endif
                elsif getSegments.phons$[n_phon] == "@"
                    cgn_type$ = "@"
                elsif getSegments.phons$[n_phon] != "@" and getSegments.phons$[n_phon] != "n" and getSegments.phons$[n_phon] != "m" and getSegments.phons$[n_phon] != "N" and getSegments.phons$[n_phon] != "[]"
                    cgn_type$ = "0"
                endif
            ...
            if p_final$ == "n" or p_final$ == "m" or p_final$ == "N"
                if p_prefinal$ == "@"
                    kal_type$ = "@n"
                elsif index(medeklinkers$, p_prefinal$) != 0
                    kal_type$ = "n"
                endif
            elsif p_final$ == "@"
                kal_type$ = "@"
            elsif p_final$ != "@" and p_final$ != "n" and p_final$ != "m" and p_final$ != "N"
                kal_type$ = "0"
            endif
            preselect_type$ = ""
            if cgn_type$ == "@n" and kal_type$ == "@n"
                preselect_type$ = "@n"
            elsif cgn_type$ == "n" or kal_type$ == "n"
                preselect_type$ = "n"
            elsif cgn_type$ == "@" and kal_type$ == "@"
                preselect_type$ = "@"
            elsif cgn_type$ == "0" and kal_type$ == "0"
                preselect_type$ = "0"
            elsif (cgn_type$ == "@" and kal_type$ == "@n") or (kal_type$ == "@" and cgn_type$ == "@n") or (cgn_type$ == "0" and kal_type$ == "n") or (kal_type$ == "0" and cgn_type$ == "n")
                preselect_type$ = "~"
            endif
            selectObject: "Table chunks"
            Set string value: id, "cgn_tran", cgn_tran$
            Set string value: id, "cgn_type", cgn_type$
            Set string value: id, "cgn_start", fixed$(cgn_start, 3)
            Set string value: id, "cgn_end", fixed$(cgn_end, 3)
            Set string value: id, "kal_tran", kal_tran$
            Set string value: id, "kal_type", kal_type$
            Set string value: id, "preselect_type", preselect_type$
        endif
    endfor
    removeObject: "TextGrid combined"
endfor
selectObject: "Table chunks"
Save as comma-separated file: tens_path$ + input_name$ + "_wrd.csv"
