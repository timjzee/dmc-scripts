...
if macintosh
    ...
    frag_path$ = "/Volumes/tensusers/timzee/af_classification/training_chunks_en/" + component$ + "/"
else
    tens_path$ = "/vol/tensusers/timzee/" + corpus$ + "/"
    audio_path$ = "/vol" + a_path$
    ort_path$ = "/vol" + o_path$
    frag_path$ = "/vol/tensusers/timzee/af_classification/training_chunks_en/" + component$ + "/"
endif
...
Read Table from comma-separated file: tens_path$ + "cgn_index_d_mono_nl.txt"
...
for s_line from 1 to n_inputlines
    ...
    if cur_name$ != wav_name$
        ...
        if corpus$ == "cgn"
            ...
            Read from file: tens_path$ + "kaldi_annot/v3/comp-" + cur_path$ + ".awd"
            ...
            Open long sound file: audio_path$ + cur_path$ + ".wav"
            wav_name$ = selected$("LongSound")
            Read from file: tens_path$ + "kaldi_annot/v3/" + cur_path$ + ".awd"
            ...
            wav_name$ = selected$("LongSound")
            Read from file: tens_path$ + "kaldi_annot/v3/" + pair_folder$ + "/" + cur_path$ + ".awd"
            Rename: wav_name$
    ...
    selectObject: "TextGrid " + wav_name$
    Extract part: cur_start, cur_end, "yes"
    num_words = Get number of intervals: tier + 2
    for w from 1 to num_words
        w_end = Get end time of interval: tier + 2, w
        p_i = Get low interval at time: tier + 3, w_end
        p_lab$ = Get label of interval: tier + 3, p_i
        Set interval text: tier + 3, p_i, p_lab$ + "#"
    endfor
    Extract one tier: tier
    selectObject: "TextGrid " + wav_name$ + "_part"
    Extract one tier: tier + 1
    selectObject: "TextGrid " + wav_name$ + "_part"
    Extract one tier: tier + 3
    selectObject: "TextGrid " + speaker$
    plusObject: "TextGrid " + speaker$ + "_META"
    plusObject: "TextGrid " + speaker$ + "_SEG"
    Merge
    Save as text file: frag_path$ + replace$(cur_path$, "/", "_", 0) + "_" + cur_start$ + "_" + cur_end$ + ".TextGrid"
    Remove
    removeObject: "TextGrid " + speaker$
    removeObject: "TextGrid " + speaker$ + "_META"
    removeObject: "TextGrid " + speaker$ + "_SEG"
