...
input_file_path = tens_path + "cgn/comp-d_en_ndl_static_final.csv"
...
def getSpeaker(fp, tier):
    tg = loadTextGrid(cgn_path + "ort/comp-d/" + fp.split("/")[-1] + ".ort")
...
#        print(skp_txt)
        ...
        with codecs.open(cgn_path + "tag/comp-d/" + f_path.split("/")[-1] + ".tag", "r", "utf-8") as h:
            tag_txt = h.read()
...
def multiProcess():
    ...
    with codecs.open(tens_path + "cgn/syntax_en_comb_comp-d.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
