...
expected_features = {"Manner": 1, "Place": 1}
...
classifiers = {
    "Manner": tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=2, hidden_units=[300, 150], model_dir=tens_path + "Manner_binary_model"),
    "Place": tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=2, hidden_units=[300, 150], model_dir=tens_path + "Place_binary_model")
}
...
#            print(af_probs)
            with open(tens_path + "pred_textgrids/" + fragment_id + "_" + af + "_binary.PitchTier", "w") as f:
...
#                max_time = float(end_time)
        ...
        tg.append(tier)
    with open(tens_path + "pred_textgrids/" + fragment_id + "_binary.TextGrid", "w") as f:
        tg.write(f)
