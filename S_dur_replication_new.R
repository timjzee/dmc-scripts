...
cat_con = matrix(c(sqrt(summary(lm(PC5 ~ type_of_s, data = s_dur_lim))$r.squared),
                   ...
                   sqrt(summary(lm(PC5 ~ corpus, data = s_dur_lim))$r.squared),
                   sqrt(summary(lm(PC1 ~ corpus, data = s_dur_lim))$r.squared),
                   sqrt(summary(lm(PC2 ~ corpus, data = s_dur_lim))$r.squared),
                   sqrt(summary(lm(PC3 ~ corpus, data = s_dur_lim))$r.squared),
                   sqrt(summary(lm(PC4 ~ corpus, data = s_dur_lim))$r.squared),
...
s_dur_lim = s_dur_lim[, c("log_s_dur", "PC1", "PC2", "PC3", "PC4", "PC5", 
                          ...
                          "type_of_s", "corpus", "speaker", "word_ort")]
na.omit(s_dur_lim)
table(s_dur_lim$corpus, s_dur_lim$type_of_s)
...
anova(intercepts2, type = 2)
lsmeans(intercepts2, pairwise~type_of_s)

# s_dur_lim = s_dur_lim[is.finite(s_dur_lim$mean_syl_dur),]
# 
# intercepts3 = lmer(log_s_dur ~ speech_rate_pron 
#                    + mean_syl_dur 
#                    + num_cons_pron
...
#                    data=s_dur_lim)
# 
# intercepts4 = lmer(log_s_dur ~ speech_rate_pron + mean_syl_dur + num_cons_pron
#                    + log_wf + prop_lex_neb_freq + p_next_w
#                    + stress_dist + next_phon_class + prev_mention + phrase_final
#                    + type_of_s + corpus
#                    + (1 | speaker) + (1 | word_ort),
