...
def stressMarkingVerbs(compound):
    num_syl_in_word = len(re.findall(r'[euoa]+(?=[^euioa]*)|(?<!e)i(?=[^euioa])', celex_verbs[compound]["compound"]))
    stress_marked = False
    syllables_in_compound = celex_verbs[compound]["phones"].split("-")
    for s in syllables_in_compound[-num_syl_in_word:]:
        if "'" in s:
            stress_marked = True
    return stress_marked
...
def getCompoundStressVerbs(compound, word):
    if not stressMarkingVerbs(compound):  # only search and replace if no stress in syllables of compound word
        if word in celex_verbs:
            if celex_verbs[word]["compound"] and celex_verbs[word]["compound"] not in compound_exceptions:
                getCompoundStressVerbs(word, celex_verbs[word]["compound"] + "en")
            else:
                pass
            new_phones = celex_verbs[word]["phones"]
            if re.search(r"^[']*[fvszSZxG]", new_phones):   # check corresponderende stemhebbende / -loze fricatieven aan het begin van gezwel (x@-'zwEl) in woorden als ettergezwel (E-t@r-G@-zwEl)
                if new_phones[0] == "'":
                    new_phones_alt = "'" + frivative_voicing[new_phones[1]] + new_phones[2:]
                else:
                    new_phones_alt = frivative_voicing[new_phones[0]] + new_phones[1:]
                if re.search(re.escape(new_phones_alt.replace("'", "")) + "(?!.*" + re.escape(new_phones_alt.replace("'", "")) + ")", celex_verbs[compound]["phones"]):
                    if not re.search(re.escape(new_phones_alt) + "(?!.*" + re.escape(new_phones_alt) + ")", celex_verbs[compound]["phones"]):  # to check whether the phones have already been updated by another parent compound
                        celex_verbs[compound]["phones"] = re.sub(re.escape(new_phones_alt.replace("'", "")) + "(?!.*" + re.escape(new_phones_alt.replace("'", "")) + ")", new_phones_alt, celex_verbs[compound]["phones"])
                else:
                    if not re.search(re.escape(new_phones) + "(?!.*" + re.escape(new_phones) + ")", celex_verbs[compound]["phones"]):  # to check whether the phones have already been updated by another parent compound
                        celex_verbs[w]["phones"] = re.sub(re.escape(new_phones.replace("'", "")) + "(?!.*" + re.escape(new_phones.replace("'", "")) + ")", new_phones, celex_verbs[compound]["phones"])
            else:
                if not re.search(re.escape(new_phones) + "(?!.*" + re.escape(new_phones) + ")", celex_verbs[compound]["phones"]):  # to check whether the phones have already been updated by another parent compound
                    celex_verbs[compound]["phones"] = re.sub(re.escape(new_phones.replace("'", "")) + "(?!.*" + re.escape(new_phones.replace("'", "")) + ")", new_phones, celex_verbs[compound]["phones"])
        else:
            remove_keys.append(compound)
            return
...
for w in celex_verbs:
    if celex_verbs[w]["compound"] and celex_verbs[w]["compound"] not in compound_exceptions:    # only search and replace (if no stress in syllables of compound word, and) if compound is not in exceptions (i.e. if it is a true compound)
        getCompoundStressVerbs(w, celex_verbs[w]["compound"] + "en")
...
for lemma in verb_dict:
    ...
    if lemma_str in celex_verbs:
        dataset_verb[lemma] = {"features": celex_verbs[lemma_str]["phones"], "class": "VERB", "freq": verb_dict[lemma]}
...
with codecs.open(tens_path + "other/pl_" + instances + ".verbs", "w", "utf-8") as f:
    for word in dataset_verb:
        syllables = dataset_verb[word]['features'].split("-")
        ...
        pl_class = [dataset_verb[word]['class']]
        compound_word_syls = len(re.findall(r'[euoa]+(?=[^euioa]*)|(?<!e)i(?=[^euioa])', celex_verbs[strip_accents(word)]["compound"])) if celex_verbs[strip_accents(word)]["compound"] else 0
        line = [word] + [str(compound_word_syls)] + features + stress + [word[-1]] + pl_class
        # tokens or types?
        if instances == "token":
            for i in range(dataset_verb[word]['freq']):
                f.write(",".join(line) + "\n")
        else:
            f.write(",".join(line) + "\n")
