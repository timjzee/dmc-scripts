...
chunk_folder = "training_chunks_n/k/"
...
#    print(phon)
    if phon == "n#":
        return [99 for af in ["n"]]
    else:
        phon = phon.strip("#")
    if phon in phones:
        label_list = [features[af][phones[phon][af]] for af in ["n"]]
        return label_list
        ...
        for old_row in range(classes.shape[0]):
            if (old_row >= 2 * frame_window) and ((old_row - frame_window) in useable_frame_indices):
                ...
                if new_labels[0] < 90:
                    ...
                    with open(af_path + "AF_n" + str(int(core) + running_cores) + ".csv", "a") as f:
