                                    ...
                                    runs=1, debug=FALSE, ...)
{
  ...
  if ("alpha" %in% param.names) {
    tmp <- list(alpha=Vm(spc, m.min) / V(spc)) # start with direct ZM estimate, cf. Evert (2004), p. 130
  ...
  model$gof <- lnre.goodness.of.fit(model, spc, n.estimated=length(param.names) + 1, m.min=m.min)
