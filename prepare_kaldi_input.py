...
if len(sys.argv) > 1:
    ...
    enable_n_weights = sys.argv[6] == "True"
    ...
    from_l = 1
    corpus = "cgn"
    index_file = "cgn_index_a_final2.txt"
    enable_n_weights = True
    with codecs.open(tens_dir + corpus + "/" + index_file, "r", "utf-8") as f:
        cgn_index = f.readlines()
    to_l = len(cgn_index) + 1

interjections = {"eih": "EI", "eikes": "EI k @ s", "mm-hu": "m h m", "oesje": "u S @", "oho": "o h o", "sjt": "S t", "uhu": "@ h @", "woh": "w o", "zuh": "z U", "zulle": "z U l @"}
...
def mark_final_n(wrd):
    pron = kaldi_lex[wrd] if wrd in kaldi_lex else oov_lex[wrd]
    if len(wrd) > 3 and enable_n_weights:
        if wrd[-2:] in ["en", "ën"] and pron[-3:] == "@ n":
            if wrd not in nnn_words:
                nnn_words.append(wrd)
            return "( {} | {}-nnn )".format(wrd, wrd)
        else:
            return wrd
    else:
        return wrd
...
#        continue
    ...
    for w_counter, w in enumerate(words, 1):
        ...
        # below is basically for mm-hu
        w_clean = re.sub(r'\*[a-z]', '', w)
        if w_clean.lower() in interjections:  # if w is an interjection
            meta[1].append("interjection")
            if w_clean.lower() not in oov_lex:
                oov_lex[w_clean.lower()] = interjections[w_clean.lower()]
            new_words.append(w_clean.lower())
            sw_l.append(w_clean.lower())
