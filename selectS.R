...
s_df = read.csv(paste(f_path, "all_s_comb.csv", sep = ""))
s_df$language = as.factor(substr(s_df$wav, 3, 4))
#s_df$cgn_comp = as.factor(substr(s_df$wav, 1, 1))
s_sub_df = s_df[s_df$language == "nl" & s_df$overlap == FALSE 
...
write.table(s_sub_df[ ,c("wav", "chan", "chunk_start", "chunk_end", "tier", "word_chunk_i", 
            ...
            file = paste(f_path, "comp-a_s.csv", sep = ""), row.names=FALSE, col.names=TRUE, sep=",", quote = FALSE)
