...
Read Table from comma-separated file: tens_path$ + "comp-a_s_ndl.csv"
...
for s_line from 1 to n_inputlines
    ...
    if cur_name$ != wav_name$
        ...
        if corpus$ == "cgn"
            ...
            wav_name$ = selected$("LongSound")
            Read from file: tens_path$ + "kaldi_annot/v1/" + pair_folder$ + "/" + cur_path$ + ".awd"
            Rename: wav_name$
        endif
    endif
...
#        start_frame = Get frame number from time: s_start
#        start_frame = ceiling(start_frame)
#        t_start_frame = Get time from frame number: start_frame
#        end_frame = Get frame number from time: s_end
#        end_frame = floor(end_frame)
#        num_frames = end_frame - start_frame + 1
#        To Matrix
#        Create Matrix: "s_frames", s_start, s_end, num_frames, time_step, t_start_frame, 1, 1, 1, 1, 1, "Matrix_'wav_name$'[1, start_frame - 1 + col]"
#        To Pitch
