...
def getNDLphons(orig_path, c_start, c_end, spkr, word_index, wndw_start, wndw_end):
    """Calls a Praat script which finds the chunk and returns info on phonetic context."""
    folder_n = orig_path.split("_part")[0].upper()
    output = subprocess.check_output([tz_path + "praat_nogui", "--run", tz_path + "GitHub/dmc-scripts/getNDLphons2.praat", orig_path, c_start, c_end, spkr, str(word_index), str(wndw_start), str(wndw_end), "ECSD/kaldi_annot/v2/" + folder_n + "/", str(n_phones)]).decode("utf-8")[:-1].split(" ")
#    print(output)
    return output
...
def multiProcess():
    ...
    with codecs.open(tens_path + "ECSD/all_s_comb_ecsd_ndl_tri.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
            with codecs.open(tens_path + "ECSD/all_s" + core_n + ".csv", "r", encoding="utf-8") as f:
                for fln, f_line in enumerate(f, 1):
                    if not (core > 0 and fln == 1):
                        g.write(f_line)
    with codecs.open(tens_path + "ECSD/ndl_ecsd_tri.csv", "w", encoding="utf-8") as g:
        for core in range(num_cores):
            core_n = str(core + 1 + running_cores)
