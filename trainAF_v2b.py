...
n_classes = {"Manner": 2, "Place": 2}

feature_dict = {
    "Manner": {1: "fricative", 0: "nil"},
    "Place": {1: "alveolar", 0: "nil"}
}

af = "Place"

_CSV_COLUMN_DEFAULTS = [[0.0] for i in range(429 + len(afs))]
_CSV_COLUMNS = ["frame" + str(i) + "mfcc" + str(j) for i in range(1, 12) for j in range(1, 40)] + afs
...
print("Loading data...")
train_data = pd.read_csv(tens_path + "AF_binary_train_labs.csv", sep=",", header=None)
valid_data = pd.read_csv(tens_path + "AF_binary_valid_labs.csv", sep=",", header=None)
...
def train_nn_classification_model(learning_rate, steps, batch_size, hidden_units, training_file, validation_file):
    ...
    classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, n_classes=n_classes[af], hidden_units=hidden_units, optimizer=my_optimizer, config=tf.contrib.learn.RunConfig(keep_checkpoint_max=1), model_dir=tens_path + af + "_binary_model")
    ...
    with open(tens_path + af + "_binary_validation_cm.csv", "w") as f:
...
classifier = train_nn_classification_model(
    ...
    training_file=tens_path + "AF_binary_train.csv",
    validation_file=tens_path + "AF_binary_valid.csv")
